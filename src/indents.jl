...
function format(x::EXPR{T}, F::FormatState) where T <: Union{cst.Begin,cst.Quote}
    doindent = is_multi_line(x, F) && T <: F.config.IndentEXPR
...
function format(x::EXPR{cst.Let}, F::FormatState) 
    doindent = is_multi_line(x, F) && cst.Let <: F.config.IndentEXPR
    nargs = length(x.args)
    for i = 1:nargs - 2
        a = x.args[i]
        offset = F.offset
...
function format(x::EXPR{cst.Try}, F::FormatState)
    doindent = is_multi_line(x, F) && cst.Try <: F.config.IndentEXPR
